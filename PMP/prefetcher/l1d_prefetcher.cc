...
struct VectorBoolHash {
    std::size_t operator()(const std::vector<bool>& vec) const {
        std::size_t seed = vec.size();
        for (bool b : vec) {
            // Convert `bool` to `int` to use standard hash
            seed ^= std::hash<int>()(b ? 1 : 0) + 0x9e3779b9 + (seed << 6) + (seed >> 2);
        }
        return seed;
    }
};
...
public:
    std::unordered_map<std::pair<uint64_t, uint64_t>, vector<bool>, pair_hash1> pdr_table_bool;
    std::unordered_map<std::pair<uint64_t, uint64_t>, vector<int>, pair_hash1> pdr_table_int;
    std::unordered_map<std::pair<uint64_t, uint64_t>, int, pair_hash1> pcr_table;
    ...
    void insert(uint64_t address, uint64_t pc, vector<bool> pattern, bool is_degrade)
    {
        ...
        if (entry)
        {
            ...
            if (entry->data.pattern[0] == max_conf) {
                if (max_value < (1 << BACKOFF_TIMES)) {
                    ...
                    pcr_table[make_pair(address, pc)]+=1;
                }
            }
            Super::rp_promote(key);
        }
        ...
        std::pair<uint64_t, uint64_t> pdr_key = make_pair(address, pc);
        if (pdr_table_int.find(pdr_key) == pdr_table_int.end()) {
            vector<int> int_pattern(pattern.size());
            transform(pattern.begin(), pattern.end(), int_pattern.begin(), [](bool b) {
                return b ? 1 : 0;
            });
            pdr_table_int[pdr_key] = int_pattern;
            pdr_table_bool[pdr_key] = pattern;
        }
        else{
            // add pattern to pdr_table_int[pdr_key]
            for(int i = 0; i < pattern.size(); i++){
                pdr_table_int[pdr_key][i] += pattern[i];
                pdr_table_int[pdr_key][i] = min(pdr_table_int[pdr_key][i], max_conf);
                pdr_table_bool[pdr_key][i] = pdr_table_bool[pdr_key][i] || pattern[i];
            }
        }
        // pdr_table_int[make_pair(address, pc)] = entry->data.pattern;
        // for(int i = 0; i < entry->data.pattern.size(); i++) {
        //     if (entry->data.pattern[i] > 0) {
        //         pdr_table_bool[make_pair(address, pc)].push_back(true);
        //     } else {
        //         pdr_table_bool[make_pair(address, pc)].push_back(false);
        //     }
        // }
    }
...
public:
    ...
    void log()
    {
        ...
        std::unordered_map<vector<int>, int, VectorIntHash> pattern_count_int;
        for (auto &e : this->opt.pdr_table_int) {
            pattern_count_int[e.second]++;
        }
        std::unordered_map<vector<bool>, int, VectorBoolHash> pattern_count_bool;
        for (auto &e : this->opt.pdr_table_bool) {
            pattern_count_bool[e.second]++;
        }
        // sum of all pattern_count_int
        int sum = 0;
        for (auto &e : pattern_count_int) {
            sum += e.second;
        }
        cout << "PDR int " << ((double)sum)/pattern_count_int.size() << endl;
        ...
        cout << "PDR bool " << ((double)sum)/pattern_count_bool.size() << endl;

        sum = 0;
        for (auto &e : this->opt.pcr_table) {
            sum += e.second;
        }
        cout << "PCR " << ((double)sum)/this->opt.pcr_table.size() << endl;
    }
